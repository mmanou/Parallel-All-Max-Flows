/*
g++ -fopenmp -std=c++11 -o solution_critical.exe solution_critical.cc
*/
...
int main(int argc, char **argv) {
    ...
    double *total_critical_time_on_cpus = (double *)calloc(highest_cpunum + 1, sizeof(double));
    double *total_critical_time_on_threads = (double *)calloc(highest_threadnum + 1, sizeof(double));
    ...
    for (ind=0; ind < NUM_GRAPHS; ind++) {
        ...
        total_critical_time_on_cpus[cpuid] += perf_critical_timings[ind];
        total_critical_time_on_threads[threadid] += perf_critical_timings[ind];
    ...
    printf("\n--CPU USAGE--\n");
    printf("CPU_ID  SOLVES  TOTAL_PARALLEL_TIME  MALLOC_TIME  CRITICAL_TIME\n");
    for (ind=0; ind < highest_cpunum + 1; ind++) {
        printf("%d %d %f %f %f\n",
            ...
            total_critical_time_on_cpus[ind]
        );
    }

    printf("\n--THREAD USAGE--\n");
    printf("THREAD_ID  CPU_ID  SOLVES  TOTAL_PARALLEL_TIME  MALLOC_TIME  CRITICAL_TIME\n");
    for (ind=0; ind < highest_threadnum + 1; ind++) {
        printf("%d %d %d %f %f %f\n",
            ...
            total_critical_time_on_threads[ind]
        );
    }
    ...
    free(total_critical_time_on_cpus);
    free(total_critical_time_on_threads);
